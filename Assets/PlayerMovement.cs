...
{
    // Physics consts
    const float STOP_VEL_THRESHOLD = 0.01f;
    ...
    {
        // Apply friction.
        currAcc_ += -currVel_ * frictionRate_;
        ...
        currVel_ += currAcc_ * Time.fixedDeltaTime;
        currVelY_ += currAccY_ * Time.fixedDeltaTime;

        currAngle_ += currVel_ * Time.fixedDeltaTime;
        currPosY_ += currVelY_ * Time.fixedDeltaTime;

        // Set velocity to 0.0f if it's close enough.
        if (Mathf.Abs(currVel_) < STOP_VEL_THRESHOLD)
            currVel_ = 0.0f;
        if (Mathf.Abs(currVelY_) < STOP_VEL_THRESHOLD)
            currVelY_ = 0.0f;
